<script>
  ...
  <div class="card bg-gray {noDisponible ? 'bg-gray-200 ' : ''} position-relative">
    ...
    {#if producto.is_promocion === true && producto.is_activo === true}
        <span class="bg-warning col-4 col-sm-3 text-white text-center position-absolute" style="z-index: 3; border-radius: 10px 0px 20px 0px;">
    ...
    <div class="row">
      <div class="col">
        <div class="card-header p-0 position-relative z-index-2" style="border-radius: 0.75rem 0px 0px 0.75rem">
          <div class="d-block blur-shadow-image cursor-pointer img-marco" >
            <img src="{fotoNoDisponible ? '/img/logo.png' : producto.url_imagen}" width="100%" height="160vh" alt="producto" class="shadow img-size {producto.is_promocion ? 'img-oferta' : 'img'} {producto.is_activo ? '' : 'img-no-activo'}" style="border-radius: 0.75rem 0px 0px 0.75rem" on:click={mostrarAlertaVisibilidad}>
          </div>
          {#if producto.is_promocion === true && producto.is_activo === true}
      ...
      <div class="col-6">
        <div class="pe-4 py-0">
          <p class="text-dark text-center text-capitalize mt-1 mb-0">{producto.nombre.length >= 14 ? producto.nombre.substring(0, 14) + "..." : producto.nombre}</p>
